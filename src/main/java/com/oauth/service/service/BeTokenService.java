...
//DefaultTokenServices github source
...
@Slf4j
@Data
public class BeTokenService extends DefaultTokenServices {
...
//        OAuth2AccessToken existingAccessToken = null;
    ...
    private OAuth2AccessToken createAccessToken(OAuth2Authentication authentication, OAuth2RefreshToken refreshToken) {
        ...
        log.info("userDetails:{}",authentication.getName());
        //自定義scopes
        boolean customScopes = true;

        EntityWrapper<OauthClientDetail> entityWrapper = new EntityWrapper<>();
        entityWrapper.eq("client_id", authentication.getOAuth2Request().getClientId());
        OauthClientDetail oauthClientDetail = oauthClientDetailService.selectOne(entityWrapper);
        if(oauthClientDetail.getAutoApprove()){
            // select authentication role_code
            List<String> roleCodes = authentication.getUserAuthentication().getAuthorities()
                    .stream().map(auth -> (auth).getAuthority()).collect(Collectors.toList());
            // roleCodes select table join role & role_scope & resource_scope
            Set<String> scopeSet = new HashSet<>();
            if (!CollectionUtils.isEmpty(roleCodes)) {
                scopeSet = resourceScopeMapper.listScopeCodeByRoleCodes(roleCodes);
            }
            //set scope
